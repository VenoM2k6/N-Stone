...
begin
   {Kurze Erklärung des Codes: Jede einzelne Anzeige skaliert mit einem Anteil der Bildschirmgröße.
    ...
    ReadLn(myfile, S);
    S:=S.Split(' ')[1];
    usernameshow1.Text:=S;
    ReadLn(myfile, S);
    S:=S.Split(' ')[1];
    manaplayer1.Text:=manaplayer1.Text+S;
    ReadLn(myfile, S);
    S:=S.Split(' ')[1];
    hpplayer1.Text:=hpplayer1.Text+S;
    ReadLn(myfile, S);
    S:=S.Split(' ')[1];
    usernameshow2.Text:=S;
    ReadLn(myfile, S);
    S:=S.Split(' ')[1];
    manaplayer2.Text:=manaplayer2.Text+S;
    ReadLn(myfile, S);
    S:=S.Split(' ')[1];
    hpplayer2.Text:=hpplayer2.Text+S;
    CloseFile(myfile);
